...
1 Переопределение метода (Overriding)
    ...
    * Модификатор доступа метода в классе-наследнике должен быть не менее строгим, чем в родительском классе.
      Например, если метод родительского класса был protected, в дочернем классе он может быть protected или private, но не public.
...
System.out.println(i); // Будут напечатаны только нечётные числа
}`
   Результат:
   1
   3
